...
async def get_fitness_advice(cb: CallbackQuery):
    ...
    if m['water_current'] < m['water_target'] * 0.5:
        ...
        advice = "🏃 Похоже, сегодня было мало движения. Как насчет 15-минутной прогулки или легкой разминки?"
    elif m['goal'] == "lose":
        advice = "🥗 Совет: Замените гарнир из риса или макарон на овощи (брокколи, кабачки). Это сэкономит вам 150-200 ккал!"
    elif m['goal'] == "gain":
        advice = "🥜 Совет: Добавьте в рацион горсть орехов или ложку арахисовой пасты — это полезные и плотные калории."
    else:
        advice = "🌟 Вы идете по плану! Старайтесь соблюдать режим сна, это важно для восстановления."

    await cb.message.answer(f"💡 <b>Совет для вас:</b>\n\n{advice}", parse_mode="HTML")
    await cb.answer()
